...
const CheckoutModal = ({ isOpen, onClose, cartItems, totalPrice, discount, clientSecret }) => {
  ...
  const handleCouponApply = async () => {
    ...
    try {
      ...
      for (let item of cartItems) {
        if (item._id) {
          try {
            const response = await axios.post('https://ecommerce-backend-pi-three.vercel.app/api/discount/coupon/validate', {
              code: couponCode,
  ...
  const handleSubmit = async (e) => {
    ...
    const submitOrder = async () => {
      ...
      try {
        const response = await axios.post('https://ecommerce-backend-pi-three.vercel.app/api/order/orders', orderData1, {
          headers: {
    ...
    try {
      ...
      await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `https://ecommerce-backend-pi-three.vercel.app/PaymentSuccess/${id}`,
        },
      });
...
const ShoppingCart = () => {
  ...
  const handleCheckout = async () => {
    ...
    if (!clientSecret) {
      ...
      try {
        const response = await fetch('https://ecommerce-backend-pi-three.vercel.app/api/product/stripe-payment', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
